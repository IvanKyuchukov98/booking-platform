...
const variantClass: Record<Variant, string> = {
    primary:
        "bg-primary text-on-primary hover:brightness-110 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:brightness-100 disabled:active:scale-100",
    secondary:
...
export function Button({
    ...
    return (
        <button
            className={`inline-flex cursor-pointer items-center justify-center gap-1 rounded-md font-medium transition-all ${variantClass[variant]} ${sizeClass[size]} ${className}`}
            {...props}
        />
    );
}
